...
class CreateIncidentUseCase:
    ...
    def __call__(self, req: CreateIncidentRequest) -> CreateIncidentResponse:
        ...
        if req.image:
            ...
            image_url = self.storage_service.save_image(req.image, image_name)
            print(f"Image saved at: {image_url}")

        self.storage_service.save(Incident(id=incident_id, description=req.description, incident_type=req.incident_type, location=req.location, title=req.title, image=image_url))
